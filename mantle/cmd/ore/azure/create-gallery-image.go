...
func init() {
	...
	sv(&coreosVersion, "coreos-version", "", "The CoreOS Version")
...
func runCreateGalleryImage(cmd *cobra.Command, args []string) error {
	...
	if coreosVersion == "" {
		fmt.Fprintf(os.Stderr, "must supply --coreos-version\n")
		os.Exit(1)
	}
	...
	galleryImage, err := api.CreateGalleryImage(galleryImageName, galleryName, resourceGroup, sourceImageId, architecture, coreosVersion)
	if err != nil {
