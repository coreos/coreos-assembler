...
def oscontainer_extract(containers_storage, tmpdir, src, dest,
    ...
    runcmd(cmd)
    runcmd(['tar', 'xf', tmp_tarball], cwd=tmpdir)
    ...
    runcmd(["ostree", "--repo=" + dest, "pull-local", repo, commit])
    if ref is not None:
        runcmd(["ostree", "--repo=" + dest, "refs", '--create=' + ref, commit])
...
def oscontainer_build(containers_storage, tmpdir, src, ref, image_name_and_tag,
    ...
    try:
        ...
        runcmd(["ostree", "--repo=" + dest_repo, "pull-local", "--disable-fsync", src, rev])
        ...
        runcmd(buildah_base_argv + ['config'] + config + [bid])
