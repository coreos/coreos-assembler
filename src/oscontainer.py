...
def oscontainer_extract(containers_storage, src, dest,
    ...
    if not tls_verify:
        ...
        tls_arg = '--tls-verify'
    if cert_dir != "":
        cert_arg = "--cert_dir={}".format(cert_dir)
    else:
        cert_arg = ""
    run_verbose(['podman', rootarg, 'pull', cert_arg, tls_arg, src])
...
def oscontainer_build(containers_storage, src, ref, image_name_and_tag,
    ...
    if push:
        ...
        if cert_dir != "":
            ...
            cert_arg = ""
        run_verbose(['podman', rootarg, 'push', cert_arg, tls_arg, image_name_and_tag])
