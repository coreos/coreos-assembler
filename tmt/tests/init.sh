#!/bin/bash
# First step in every tmt plan. Initializes the coreos-assembler workspace by
# cloning the Fedora CoreOS config repo (testing-devel branch) into COSA_DIR,
# which all subsequent build and test steps depend on.
set -eEuo pipefail
set -x
